...
import { IconTrash, IconPencil, IconChevronUp, IconChevronDown, IconBuildingSkyscraper } from "@tabler/icons-react";
...
export default function TrustedByTable({
  items,
  onEditClick,
  onDeleteClick,
  onToggleVisible,
  ...
  items: TrustedBy[];
  onEditClick: (item: TrustedBy) => void;
  onDeleteClick: (item: TrustedBy) => void;
  onToggleVisible: (item: TrustedBy) => void;
      ...
      <table className="w-full min-w-[600px] text-left text-sm">
        <thead>
          <tr className="border-b border-white/10 bg-white/[0.02] text-xs uppercase tracking-wide text-base-gray">
            ...
            <th className="px-4 py-3 font-medium">{dict.table.brand}</th>
              ...
              <td className="px-4 py-3">
                <div className="flex items-center gap-3">
                  <div className="flex h-9 w-9 shrink-0 items-center justify-center overflow-hidden rounded-lg bg-white/[0.05]">
                    {item.logoUrl ? (
                      <Image src={item.logoUrl} alt="" width={36} height={36} className="h-full w-full object-contain" />
                    ) : (
                      <IconBuildingSkyscraper size={18} className="text-purple-light" />
                    )}
                  </div>
